...
the first page. User directive (2026-08-14): the index must keep loading
entries asynchronously. Full reloads page the whole query result:
`notmuch search --limit=N --offset=M`, each page merged into the view
with a ViewDiff before the next fetch, until a short page ends the fill.
Each page is its own progress batch (spec section 8).

**Files:**
- Modify: `src/notmuch/backend.go` (Backend.Query signature + Action.Offset)
- Modify: `src/notmuch/cli.go` (--offset argv), `src/notmuch/cgo.go` (signature)
- Modify: `src/notmuch/worker.go` (Action.Offset field, handle passes it)
- Modify: `src/app/refresh.go` (fullReload page loop + mergeSorted)
- Test: `src/app/refresh_test.go` (fill test), `src/notmuch/worker_test.go` (fake signatures)

- [ ] **Step 1: Write the failing tests**

`src/app/refresh_test.go` - extend the fake worker (read the existing
fakeWorker first; give it offset/limit-aware ActQuery serving) and add:

```go
func TestFullReloadPages(t *testing.T) {
	// 2500 stubs: page 0 returns 1000, page 1 returns 1000, page 2 returns 500, page 3 empty
	// -> the view ends with all 2500 merged and three ViewDiff events.
}
```

Assert: final row count 2500, ViewDiff published per page (3), and the
snapshot is the full merged set. Also a short-query test: 0 stubs -> one
empty merge, no infinite loop.
...
`--offset=` when offset > 0 (argv, never shell strings - F4); the 3
worker_test fakes gain the param.

`src/app/refresh.go` fullReload - page loop with a linear merge (never
re-sort the whole snapshot per page - quadratic in the result size):
...
// sorted pages, and re-sorting the whole snapshot per page is
// quadratic in the result size.
func mergeSorted(a, b []*core.Thread) []*core.Thread { ... }
```

- [ ] **Step 4: Run to verify they pass**

`go test ./...` all pass; `go test -race ./app/` clean.
