...
need to batch all at once"): the fill is the INDEX read - search pages
carry thread summaries (thread id, date, authors, subject, tags) with
zero file opens, so the whole list loads in seconds; the per-thread
round trip that was the load wall is gone because the fill never
fetches threads. Step two is per-message data (ids, references, paths):
after the fill, fillViewport hydrates the visible window (rows 1-40,
the cache job's scanPage) through the budgeted per-thread fetch - the
only file opens in the load path - and the rest loads on open (R13).
The index rows ARE search summaries: a stub message (empty id) renders
and is cursorable, anchored by row index (SetCursorIndex); the hydrate
merge removes the stub and inserts the real messages (cursor lands on
the thread root). Stub rows cannot be staged (the apply path needs a
real message id; one guard in the TUI, same shape as the ghost guard).
The hydrate reports no progress (report=false) - it must not clobber
the fill's count-total bar with its own small total.
...
the result size); search summaries group into STUB threads (one message
per thread, empty id); after the last page, fillViewport hydrates the
visible window through fetchThreads; fetchThreads publishes
`Progress{Job: "refresh", View: r.view.Name, Done: base + done,
Total: total}` with the fallback `if total <= 0 { total = len(ids);
base = 0 }` and a `report bool` that silences the publishes (the
hydrate must not clobber the count-total bar):
...
CursorIndex falls back to it when the cursor id is empty. Stage/Undo
guard on empty ids (`row.Msg.ID == ""`), same shape as the ghost guard.
