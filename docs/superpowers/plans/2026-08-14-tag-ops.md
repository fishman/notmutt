...
// removed by folder moves.
//
// newTags is the display arm (stage-time rendering); resolved is the
// apply arm - the minimal op set (symmetric difference, sorted by tag)
// turning the input tags into newTags. A net no-op yields an empty
// resolved arm.
