...
Replace `TestToggleRead` and `TestToggleReadConcurrent` in `src/tui/model_test.go` with the staged contract; add the new tests. The `model()` fixture must now set groups AND pass the key map to the three-argument New (spec section 7: bindings are config data, the model dispatches on action names):
...
(spec section 7: the plan's key letters were examples, not the
mechanism). The model takes the key->action map at construction and
dispatches on the ACTION, never the literal rune:
...
`New(view, ch, keys map[string]string)` stores the map. In Update, replace the literal-rune switch with action dispatch (key lookup: `action := m.keys[string(msg.Runes)]`; unknown key falls through to no action):
...
Replace `toggleRead`/`removeTag`/`stageKey` with per-action staging:
...
`src/config/config.go` - the binding data (defaults in Default(), shape
validation in validate; action-name validation stays in the app wiring
because the vocabulary lives in tui):
...
validate: each context's map must be non-empty; each key and each
action value must be non-empty (blank entries error like tag-groups).

`src/app/app.go` - wire: after load, validate every value in
Bindings["index"] against tui.Actions (unknown action = startup error),
and pass the map to New(view, ch, keys).
...
Run: `cd /home/user/git/opencode/notmutt/src && go test ./... && go vet ./...`
Expected: all pass, vet clean. Grep for stragglers: `grep -rn "SetTagOpHandler\|toggleRead\|removeTag\|onTagOp" src/` - must be empty.

- [ ] **Step 5: Commit**

```bash
git add src/tui/model.go src/tui/hooks.go src/tui/model_test.go src/app/app.go
git commit -m "feat(tui): stage tag ops with r/a/d/u and apply with \$"
```
