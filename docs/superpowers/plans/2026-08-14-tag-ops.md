...
Replace `TestToggleRead` and `TestToggleReadConcurrent` in `src/tui/model_test.go` with the staged contract; add the new tests. The `model()` fixture must now set groups (add `core` import already present):
...
`src/tui/model.go` - keys. Replace the `case "t":` block:
...
Delete `toggleRead` and `removeTag`; add `stageKey` (place it where toggleRead was):
