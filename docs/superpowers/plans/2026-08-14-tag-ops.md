...
### Task 8b: progressive page fill (user requirement, mid-M2)

The refresher currently loads ONE page of the view query (page = 1000) and then only syncs lastmod changes - the list never advances past
the first page. User directives (2026-08-14): the index must keep loading
entries asynchronously; the bar must reflect the query's real total even
as it updates periodically (count up front, then accumulate); progress is
scoped PER VIEW (accounts, inbox, unread, sent, drafts are all views that
can be loaded); and the first page must be 200 so the first paint is fast
(200/1000, not 1000/1000). Full reloads page the whole query result:
`notmuch count` fixes the bar's Total, then `notmuch search --limit=N
--offset=M`, each page merged into the view with a ViewDiff before the
next fetch, until a short page ends the fill. Count failure degrades to
per-page totals with the base reset (Done restarts each page; it never
exceeds the total).
...
(matching Count). The CLI serves the page as one `notmuch search`
subprocess (thread summaries: thread id, date, authors, subject, tags
- the CLI has no content-free per-message dump, so batching there is
just one call per page); the cgo backend serves it as the NATIVE batch
- the in-process threads iterator, skip/limit on thread positions,
each thread's messages mapped directly into core.Message from the DB
header cache (ids, references, from, subject, date, tags; references
and in-reply-to are Xapian values, no file opens), zero subprocesses
(user directive: "it's odd to use batch with the cli rather than with
...
**Files:**
- Modify: `src/notmuch/backend.go` (Backend.Query signature + Action.Offset, new Backend.Count + Action.Count kind)
- Modify: `src/notmuch/cli.go` (--offset argv, count --output=threads), `src/notmuch/cgo.go` (signatures)
- Modify: `src/notmuch/worker.go` (Action.Offset + ActCount, handle passes both)
- Modify: `src/core/bus.go` (Progress{Job, View, Done, Total}; latest-progress snapshot map)
- Modify: `src/app/refresh.go` (fullReload page loop + mergeSorted + count-first totals + firstPage const)
...
fakeWorker first; give it offset/limit-aware ActQuery serving and
ActCount stubbing) and add:

```go
func TestFullReloadPages(t *testing.T) {
	// 2500 stubs, page 0 limit 200, pages 1-2 limit 1000, page 3 limit
	// 1000 returns 300: four fetches (200/1000/1000/300), the view ends
	// with all 2500 merged, snapshot sorted, and LatestProgress
	// ("refresh", "inbox") == {Done: 2500, Total: 2500} (count-first).
}
```

Assert: four ActQuery calls, final row count 2500, snapshot sorted, and
the bus snapshot `LatestProgress("refresh", "inbox")` ends at
Done 2500 / Total 2500 (the count query fixes the total; Done
accumulates across pages, never a per-page reset). Also:
`TestFullReloadCountFailure` - `setCountErr` -> fill still completes,
and the last page publishes its own batch total with the base reset
({Done: 300, Total: 300} for the 300-thread tail); and a short-query
test: 0 stubs -> one empty merge, no infinite loop.
...
`Query(ctx, query string, limit, offset int)`; CLI appends
`--offset=` when offset > 0 (argv, never shell strings - F4); new
...
`src/app/refresh.go` fullReload - count first, then the page loop with a
linear merge (never re-sort the whole snapshot per page - quadratic in
the result size); each page groups into threads (CLI: one STUB thread
per thread id - a summary message with an empty id; cgo: real threads
from per-message rows); no per-thread fetch anywhere in the load path:
...
Note: `offset += limit` runs BEFORE `limit = r.page` - incrementing by
the bumped limit skips pages (a real bug caught in review).
...
```bash
git commit -m "feat(refresh): page the full query result in asynchronously"
```
