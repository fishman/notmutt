...
The refresher currently loads ONE page of the view query (`Limit: r.page`,
200) and then only syncs lastmod changes - the list never advances past
...
func TestFullReloadPages(t *testing.T) {
	// 250 stubs: page 0 returns 200, page 1 returns 50, page 2 empty
	// -> the view ends with all 250 merged and two ViewDiff events.
}
```

Assert: final row count 250, ViewDiff published per page (2), and the
