...
// TestLoadGlyphSet pins the [ui] glyph-set preset: ascii (the default)
// keeps the ASCII tree glyphs, "utf-8" swaps in box-drawing, an
// explicit [ui.glyphs] tree key overrides the preset, and an unknown
...
func TestLoadGlyphSet(t *testing.T) {
	...
	if cfg.UI.Glyphs.Tree != "+ " || cfg.UI.Glyphs.TreeChild != "| " ||
		...
		t.Fatalf("ascii glyphs must be the default: %+v", cfg.UI.Glyphs)
	}
	...
	if cfg.UI.Glyphs.Tree != "▸ " || cfg.UI.Glyphs.TreeChild != "│ " ||
		cfg.UI.Glyphs.TreeBranch != "├─" || cfg.UI.Glyphs.TreeLeaf != "└─" {
		t.Fatalf("utf-8 glyph-set must swap in the box-drawing tree glyphs: %+v", cfg.UI.Glyphs)
	}
	cfg, err = Load(write(t, "[ui]\nglyph-set = \"utf-8\"\n\n[ui.glyphs]\ntree = \"> \"\n"))
	if err != nil {
		t.Fatal(err)
...
`))
	...
	res, _ = cfg.Theme.Resolved(cfg.Palette, "light")
