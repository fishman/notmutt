...
// replayable ops; positions are in the current list at apply time, so
// Apply(old, ops) == new. A remove+insert of the same key collapses
// into a Move (second pass, still O(n+m)). Matched keys keep the old
// element (values not updated) - callers reconcile fields from the
// snapshot. less must be a strict total order on keys.
...
// collapseMoves turns an ADJACENT remove-then-insert pair of the same
// key into a Move. Only adjacent pairs collapse: the walk advances via
// matched pairs alone, so From/To reference the same frame and the
// collapsed op equals the original two (the property test is the gate).
// Any intervening op shifts the frames; the reverse order (an element
// rising) never collapses - both stay as remove+insert churn.
...
// Apply replays ops in order over items; the result equals the list the
// ops were diffed from. It mutates items' backing array in place -
// callers must use the returned slice and not retain or alias the
// original header.
