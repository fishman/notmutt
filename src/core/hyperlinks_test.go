...
// The aerc hyperlinks_test.go table, carried verbatim as the port's
// regression spec. Output is sorted, so order is part of the contract.
func TestLinks(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			links: []string{"http://git.sr.ht/~rjarry/aerc", "https://aerc-mail.org"},
		},
		{
			name:  "rfc",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got := Links(test.text, test.html)
			if !slices.Equal(got, test.links) {
				t.Fatalf("Links(%q, %v) = %v, want %v", test.text, test.html, got, test.links)
			}
		})
	}
}
