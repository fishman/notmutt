...
func TestRenderThread(t *testing.T) {
	...
	lines, _, _, err := RenderThread(msgs, core.RenderHTML, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderThreadAttachment(t *testing.T) {
	...
	lines, _, _, err := RenderThread([]core.Message{{ID: "m1", ThreadID: "t1", Paths: []string{p}}}, core.RenderHTML, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	joined := joinText(lines)
...
func TestRenderThreadNoPath(t *testing.T) {
	lines, _, _, err := RenderThread([]core.Message{{ID: "m1", ThreadID: "t1"}}, core.RenderHTML, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	joined := joinText(lines)
...
func TestRenderThreadEmpty(t *testing.T) {
	if _, _, _, err := RenderThread(nil, core.RenderHTML, false, 0, false, false, "", false, nil); err == nil {
...
func TestRenderThreadBodyTruncated(t *testing.T) {
	...
	lines, _, _, err := RenderThread([]core.Message{{ID: "m1", ThreadID: "t1", Paths: []string{fixture(t, big)}}}, core.RenderHTML, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	joined := joinText(lines)
...
func TestRenderThreadTextView(t *testing.T) {
	...
	lines, _, _, err = RenderThread(msgs, core.RenderPlain, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	lines, _, _, err = RenderThread(msgs, core.RenderSource, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderThreadSourceView(t *testing.T) {
	...
	lines, mime, _, err := RenderThread(msgs, core.RenderSource, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	lines, mime, _, err = RenderThread(msgs2, core.RenderPlain, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	out := joinText(lines)
	...
	// views are distinct
	lines, mime, _, err = RenderThread(msgs2, core.RenderSource, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderThreadMimeLabel(t *testing.T) {
	...
	for _, mode := range []core.RenderMode{core.RenderPlain, core.RenderHTML, core.RenderSource} {
		lines, mime, _, err := RenderThread(msgs, mode, false, 0, false, false, "", false, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestRenderThreadHeaders(t *testing.T) {
	...
	lines, _, _, err := RenderThread(msgs, core.RenderPlain, true, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
	out := joinText(lines)
...
func TestEnvelopeShowsCc(t *testing.T) {
	...
	lines, _, _, err = RenderThread(msgs2, core.RenderPlain, false, 0, false, false, "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderThreadHeadersHTML(t *testing.T) {
	...
	for _, mode := range []core.RenderMode{core.RenderPlain, core.RenderHTML} {
		lines, _, _, err := RenderThread(msgs, mode, true, 0, false, false, "", false, nil)
		if err != nil {
			t.Fatal(err)
		}
		out := joinText(lines)
