...
func TestKeyPressOfCtrl(t *testing.T) {
	advanced, ok := keyPressOf(tcell.NewEventKeyEx(tcell.KeyRune, "f", tcell.ModCtrl, true, 0, 1))
	if !ok || advanced.String() != "ctrl+f" {
		t.Fatalf("advanced ctrl+f must map identically, got %v (ok %v)", advanced, ok)
	}
	plain, ok := keyPressOf(tcell.NewEventKey(tcell.KeyRune, "j", tcell.ModNone))
...
func TestKeyPressOfShift(t *testing.T) {
	kitty, ok := keyPressOf(tcell.NewEventKeyEx(tcell.KeyRune, "h", tcell.ModShift, true, 0, 1))
	...
	legacy, ok := keyPressOf(tcell.NewEventKey(tcell.KeyRune, "H", tcell.ModNone))
	...
	shifted, ok := keyPressOf(tcell.NewEventKeyEx(tcell.KeyRune, "H", tcell.ModShift, true, 0, 1))
	if !ok || shifted.Text != "H" {
		t.Fatalf("shift+uppercase must stay H, got %v (ok %v)", shifted, ok)
	}
	// non-letter with shift keeps its text
	dig, ok := keyPressOf(tcell.NewEventKeyEx(tcell.KeyRune, "1", tcell.ModShift, true, 0, 1))
	if !ok || dig.Text != "1" {
